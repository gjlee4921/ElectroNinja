...
def delete_all_subdirs(parent_dir: str) -> None:
    """
    Deletes all directories (and their contents) inside the specified parent directory.
    Does not delete files at the top level.
    """
    parent = Path(parent_dir)
    if not parent.is_dir():
        raise ValueError(f"Not a valid directory: {parent_dir}")
...
if __name__ == "__main__":
    delete_all_subdirs(r"data\\output")
    main()
